...
_extern MSImageRef MSGetImageByName(const char *file){
	MSImageRef imageRef = NULL;

	imageRef = dlopen(file, RTLD_NOW);
	if(!imageRef){
		MSLog(0, "MSGetImageByName fail, name=%s", file);
	}
	return imageRef; 
}
...
_extern void *MSFindSymbol(MSImageRef image, const char *name){
	void *sym = NULL;
	sym = dlsym((void *)image, name);

	if(!sym){
		MSLog(0, "MSFindSymbol fail, image=%p, name=%s", image, name);
	}
	return sym;
}
